...
const defaultEditing: EditingPreferences = {
...
function mockAppConfig(legacy: SidebarPreferences): AppConfig {
  ...
  return {
    version: 1,
    sidebars: parsed.sidebars ?? legacy,
...
function addMockSession(status: ConnectionStatus) {
  const existing = mockSessions.find(
    (item) => item.driver === status.driver && item.path === status.path && item.readOnly === status.readOnly,
  )
...
function previewScripts(): Record<string, { content: string; modified: string }> {
  try {
    return JSON.parse(localStorage.getItem(previewScriptsKey) ?? '{}')
  } catch {
    return {}
  }
}
...
const mock: Backend = {
  ...
  async ListDatabaseSessions() {
    return [...mockSessions]
  },
  async OpenPostgresSession(config) {
    return addMockSession(await this.ConnectPostgres(config))
  },
  async OpenSavedSession(id, password) {
    return addMockSession(await this.ConnectSavedConnection(id, password))
  },
  async ChooseSQLiteSession() {
    return addMockSession(await this.ChooseSQLiteFile())
  },
  async OpenDemoSession() {
    return addMockSession(await this.ConnectDemo())
  },
  async CloseDatabaseSession(id) {
    mockSession(id)
    mockSessions = mockSessions.filter((item) => item.id !== id)
  },
  async ListDatabases(id) {
    const session = mockSession(id)
    return session.driver === 'PostgreSQL'
      ? [...new Set([session.database, 'analytics', 'inventory', 'postgres', ...mockCreatedDatabases])].sort()
      : [session.database]
  },
  ...
  async OpenDatabase(id, database) {
    const session = mockSession(id)
    return addMockSession({
      ...session,
      database,
      path: session.path.slice(0, session.path.lastIndexOf('/') + 1) + database,
    })
  },
  async SessionListTables(id) {
    mockSession(id)
    return this.ListTables()
  },
  async SessionCountTableRows(id, schema, table) {
    mockSession(id)
    return this.CountTableRows(schema, table)
  },
  async SessionGetTableSchema(id, schema, table) {
    mockSession(id)
    return this.GetTableSchema(schema, table)
  },
  async SessionGetTableIndexes(id, schema, table) {
    mockSession(id)
    return this.GetTableIndexes(schema, table)
  },
  async SessionGetTableData(id, ...args) {
    mockSession(id)
    return this.GetTableData(...args)
  },
  async SessionExecuteQuery(id, query) {
    mockSession(id)
    return this.ExecuteQuery(query)
  },
  async SessionExecuteQueryTracked(id, operationID, query) {
    mockSession(id)
    return mockTracked(operationID, () => this.ExecuteQuery(query))
  },
  async SessionExecuteScriptStatement(id, query) {
    mockSession(id)
    return this.ExecuteScriptStatement(query)
  },
  async SessionExecuteScriptStatementTracked(id, operationID, query) {
    mockSession(id)
    return mockTracked(operationID, () => this.ExecuteScriptStatement(query))
  },
  async SessionCancelOperation(id, operationID) {
    mockSession(id)
    const operation = mockOperations.get(operationID)
    operation?.abort()
    return Boolean(operation)
  },
  async SessionApplyChanges(id, schema, table, operations) {
    mockSession(id)
    return this.ApplyChanges(schema, table, operations)
  },
  async SessionApplyChangesTracked(id, operationID, schema, table, operations) {
    mockSession(id)
    return mockTracked(operationID, () => this.ApplyChanges(schema, table, operations))
  },
  async SessionPreviewDatabaseBackup(id) {
    const session = mockSession(id)
    ...
    return {
      ...
      tables: await Promise.all(
        tables
          ...
          .map(async (table) => {
            const data = await this.GetTableData(table.schema, table.name, 5, 0, '', '', '')
            return {
              schema: table.schema,
              name: table.name,
  ...
  async SessionExportTables(_id, tables) {
    return { path: 'preview.json', tables: tables.length, rows: 0, skipped: 0 }
  },
  async SessionChooseTableImport() {
    throw new Error('File selection is available in the desktop app.')
  },
  async SessionImportTable() {
    return { path: 'preview.csv', tables: 1, rows: 0, skipped: 0 }
  },
  async SessionTruncateTables(_id, tables) {
    return tables.length
  },
  async SessionTruncateTablesTracked(id, operationID, tables) {
    mockSession(id)
    return mockTracked(operationID, () => this.SessionTruncateTables(id, tables))
  },
  async SessionScriptWorkspacePath(id) {
    mockSession(id)
    return '~/Library/Application Support/QueryNest/projects/preview'
  },
  async SessionListScripts(id) {
    ...
    return Object.keys(scripts)
      ...
      .map((name) => previewScriptFile(name, scripts[name]))
  },
  ...
  async SessionRenameScript(id, from, to) {
    ...
    if (source !== target) {
      scripts[target] = scripts[source]
      delete scripts[source]
    }
    savePreviewScripts(scripts)
  ...
  async GetStatus() {
    return {
      id: '',
      database: mockConnected ? 'querynest-demo' : '',
      connected: mockConnected,
      name: mockConnected ? 'querynest-demo' : '',
      path: mockConnected ? '~/querynest-demo.db' : '',
      driver: 'SQLite',
      readOnly: false,
    }
  },
  ...
  async TestPostgresConnection() {
    throw new Error('Connection testing is available in the desktop app.')
  },
  async ConnectDemo() {
    mockConnected = true
    return this.GetStatus()
  ...
  async ListTables() {
    return [
      { schema: 'public', name: 'customers', type: 'table', rows: -1 },
      { schema: 'public', name: 'orders', type: 'table', rows: -1 },
      { schema: 'public', name: 'active_customers', type: 'view', rows: -1 },
    ]
  },
  async CountTableRows(_schema, table) {
    return table === 'customers' ? 8 : 5
  },
  ...
  async GetTableIndexes(_schema, table) {
    return table === 'orders'
      ? [
          ...
          { name: 'orders_pkey', type: 'btree', columns: ['id'], unique: true, primary: true, partial: false },
        ]
      : [
          {
          ...
          { name: 'customers_pkey', type: 'btree', columns: ['id'], unique: true, primary: true, partial: false },
        ]
  },
  async GetTableData(_schema, table, limit, offset, filter, sortColumn, sortDirection) {
    let rows =
      table === 'orders'
        ? [
            [1001, 1, 249, 'USD', 'paid', '2026-09-01 10:20:00'],
            [1002, 2, 89.5, 'USD', 'pending', '2026-09-01 12:05:00'],
            [1003, 1, 510, 'USD', 'paid', '2026-09-02 08:41:00'],
            [1004, 4, 120, 'EUR', 'refunded', '2026-09-02 14:12:00'],
            [1005, 6, 75.25, 'USD', 'paid', '2026-09-03 09:06:00'],
          ]
        : demoRows
    const columns =
      table === 'orders'
        ? ['id', 'customer_id', 'total', 'currency', 'status', 'ordered_at']
        : ['id', 'name', 'email', 'company', 'status', 'created_at']
    if (filter)
      rows = rows.filter((row) => row.some((value) => String(value).toLowerCase().includes(filter.toLowerCase())))
    if (sortColumn) {
  ...
  async ExecuteScriptStatement(query) {
    if (/^\s*(?:select|with|explain|show|values|table|pragma)\b/i.test(query) || /\breturning\b/i.test(query))
      return this.ExecuteQuery(query)
    ...
    return {
      ...
      durationMs: 3,
      message: affected ? `Affected ${affected} row(s)` : 'Statement executed',
    }
  },
  async UpdateCell() {},
  async ApplyChanges(_schema, _table, operations) {
...
export function databaseApi(id: string) {
  ...
  return {
    ...
    GetTableData: (
      ...
      sortColumn: string,
      sortDirection: string,
    ) => backend.SessionGetTableData(id, schema, table, limit, offset, filter, sortColumn, sortDirection),
    ExecuteQuery: (operationID: string, query: string) => backend.SessionExecuteQueryTracked(id, operationID, query),
    ExecuteScriptStatement: (operationID: string, query: string) =>
      backend.SessionExecuteScriptStatementTracked(id, operationID, query),
    CancelOperation: (operationID: string) => backend.SessionCancelOperation(id, operationID),
    ApplyChanges: (operationID: string, schema: string, table: string, operations: RowOperation[]) =>
      backend.SessionApplyChangesTracked(id, operationID, schema, table, operations),
