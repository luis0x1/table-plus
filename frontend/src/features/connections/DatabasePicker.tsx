...
import { Alert, Check, ChevronDown, Database, Plus, Refresh, Search, X } from '../../components/ui/icons'
...
export default function DatabasePicker(props: { status: ConnectionStatus; onSelect: (database: string) => void }) {
  ...
  createEffect(() => {
    ...
    api()
      .ListDatabases(id)
      .then((next) => {
        if (!cancelled) setDatabases(next ?? [])
      })
      .catch((e) => {
  ...
  return (
    <div class="database-picker" ref={container}>
      <button
        class="database-picker-trigger"
        ref={trigger}
        aria-label={`Choose database: ${props.status.database}`}
        aria-haspopup="dialog"
        aria-expanded={open()}
        title={props.status.database}
      ...
      <Show when={open()}>
        <section class="database-menu" role="dialog" aria-label="Databases">
          <header>
            <b>Databases</b>
            <span>{props.status.driver === 'PostgreSQL' ? props.status.name : 'SQLite'}</span>
          </header>
          ...
          <Show
            when={!loading()}
            fallback={
              <div class="database-menu-message" role="status">
                <Refresh size={14} class="spin" /> Loading databases…
              </div>
            }
          >
            <Show
              when={!error()}
              fallback={
                <div class="database-menu-message error" role="alert">
                  <Alert size={15} />
                  <span>{error()}</span>
                  <button class="secondary" onClick={() => setRetry((value) => value + 1)}>
                    Retry
                  </button>
                </div>
              }
            >
              <div class="database-menu-list">
                <For each={filtered()}>
                  {(name) => (
                    <button
                      class={name === props.status.database ? 'active' : ''}
                      title={name}
                      aria-pressed={name === props.status.database}
                      onClick={() => {
                        setOpen(false)
                        trigger?.focus()
                        props.onSelect(name)
                    ...
                    >
                      <Database size={15} />
                      <span>{name}</span>
                      <Show when={name === props.status.database}>
                        <Check size={14} />
                      </Show>
                    </button>
                  )}
                ...
                <Show when={!filtered().length}>
                  <p class="database-menu-message">No databases found.</p>
                </Show>
              </div>
              <Show when={props.status.driver === 'PostgreSQL'}>
                <Show
                  ...
                  fallback={
                    <form class="database-create-form" onSubmit={createDatabase}>
                      ...
                      <button type="submit" class="primary" disabled={!createName().trim() || submitting()}>
                        {submitting() ? <Refresh size={13} class="spin" /> : <Plus size={13} />}{' '}
                ...
                >
                  <button
                    class="database-create-button"
                    disabled={props.status.readOnly}
                    title={
                      props.status.readOnly
                        ? 'Reconnect with editing enabled to create a database'
