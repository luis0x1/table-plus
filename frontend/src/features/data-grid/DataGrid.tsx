...
export function buildDraftGrid(
  ...
  page: number,
): { data: TableData; meta: GridRowMeta[] } {
  ...
  data.rows.forEach((baseRow, baseIndex) => {
    ...
    const row = data.columns.map((column, index) =>
      update && column in update.values ? update.values[column] : baseRow[index],
    )
    rows.push(row)
    meta.push({
      id,
      kind: deleted ? 'delete' : update ? 'update' : 'clean',
      canEdit: !deleted && Object.keys(primaryKey).length > 0,
      primaryKey,
      baseIndex,
    })
  })
...
export default function DataGrid(raw: DataGridProps) {
  ...
  const [editing, setEditing] = createSignal<{ row: number; column: string; text: string; original: unknown } | null>(
    null,
  )
  ...
  createEffect(
    on(
      () => [props.layoutKey, props.data.columns.join('\u0000')] as const,
      ([layoutKey]) => {
  ...
  createEffect(() => {
    ...
    if (layoutKey && currentOrder.length)
      localStorage.setItem(
        `querynest:grid:${layoutKey}`,
        JSON.stringify({ order: currentOrder, widths: currentWidths }),
      )
  })
  ...
  createEffect(
    on(
      () => [virtualized(), props.compact] as const,
      ([isVirtual]) => {
        if (!isVirtual) return
        queueMicrotask(() => {
  ...
  const shown = () =>
    order()
      .map((column) => ({ column, source: props.data.columns.indexOf(column) }))
  ...
  function resize(event: PointerEvent, column: string) {
    ...
    const initial = columnWidth(column)
    const move = (next: PointerEvent) =>
      setWidths((current) => ({ ...current, [column]: Math.max(72, Math.min(600, initial + next.clientX - start)) }))
    const up = () => {
  ...
  async function commitEdit() {
    ...
    try {
      ...
      /* Parent surfaces the update error while keeping the editor open. */
    } finally {
      setSaving(false)
    }
  }
    ...
    <Show when={props.data.columns.length} fallback={<div class="empty-grid">No result columns</div>}>
      <div ref={scroller} class={`grid-scroll ${props.compact ? 'compact' : ''}`}>
        <table
          ...
          aria-rowcount={virtualized() ? totalRows() + 1 : undefined}
        >
          <colgroup>
            <col class="row-col" style={{ width: `${rowNumberWidth()}px` }} />
            <Index each={shown()}>{(item) => <col style={{ width: `${columnWidth(item().column)}px` }} />}</Index>
          </colgroup>
          <thead>
            <tr>
                ...
                {(item, columnIndex) => (
                  ...
                  >
                    <Show when={props.layoutKey && columnIndex > 0}>
                      <i
                        {...resizerFor(() => shown()[columnIndex - 1].column)}
                        class="column-resizer column-resizer-left"
                      />
                    </Show>
                    <span>{item().column}</span>
                    <Show when={props.sortColumn === item().column}>
                      {props.sortDirection === 'asc' ? <ArrowUp size={13} /> : <ArrowDown size={13} />}
                    </Show>
          ...
          <tbody ref={body}>
            <Show when={padTop() > 0}>
              <tr class="grid-spacer" aria-hidden="true">
                <td colspan={shown().length + 1} style={{ height: `${padTop()}px` }} />
              </tr>
            </Show>
            <Index each={visibleRows()}>
              {(row, offset) => {
                ...
                return (
                  <tr
                    class={`draft-${meta()?.kind ?? 'clean'} ${selected().has(rowID()) ? 'selected' : ''} ${rowIndex() % 2 ? 'even' : ''}`}
                    style={{ '--stagger': String(offset) }}
                    aria-rowindex={virtualized() ? rowIndex() + 2 : undefined}
                  >
                    <td class="row-number">
                      <button
                        class="row-selector"
                        disabled={!props.onSelect || (meta()?.kind !== 'insert' && !meta()?.canEdit)}
                        onClick={() => {
                          const current = meta()
                          if (current) props.onSelect?.(current.id)
                        }}
                      >
                        {selected().has(rowID()) ? <Check size={11} /> : props.rowOffset + rowIndex() + 1}
                      </button>
                    </td>
                    <Index each={shown()}>
                      {(item) => {
                        ...
                        const active = () => {
                          const state = editing()
                          return state && state.row === rowIndex() && state.column === item().column ? state : null
                        }
                            ...
                            <Show
                              ...
                              fallback={
                                <Show
                                  when={json()}
                                  fallback={
                                    <span class={isStatus() ? `status-pill ${text().toLowerCase()}` : ''}>
                                      {formatCell(value())}
                                    </span>
                                  }
                                >
                                  <button
                                    class="json-cell"
                                    title={JSON.stringify(JSON.parse(json()!))}
                                    onClick={() =>
                                      setJsonCell({ row: rowIndex(), column: item().column, value: value() })
                                    }
                                  >
                                    <Code size={13} />
                                    <span class="json-preview">{JSON.stringify(JSON.parse(json()!))}</span>
                                  </button>
                                </Show>
                              }
                            >
            ...
            <Show when={padBottom() > 0}>
              <tr class="grid-spacer" aria-hidden="true">
                <td colspan={shown().length + 1} style={{ height: `${padBottom()}px` }} />
              </tr>
            </Show>
          </tbody>
        </table>
        <Show when={jsonCell()}>
          {(cell) => (
            <JsonModal
              value={cell().value}
              editable={props.editable && (props.rowMeta[cell().row]?.canEdit ?? true)}
              onClose={() => setJsonCell(null)}
              onSave={
                props.onUpdate
                  ? async (value) => {
                      await props.onUpdate!(cell().column, cell().row, value)
                      setJsonCell(null)
                    }
                  : undefined
              }
...
function JsonModal(props: {
  ...
  return (
    <div
      class="modal-backdrop json-backdrop"
      onMouseDown={(event) => {
    ...
    >
      <section class="json-modal" role="dialog" aria-modal="true" aria-label="JSON viewer">
        <header>
          ...
          <div>
            <button class="secondary" onClick={() => navigator.clipboard?.writeText(text())}>
              <Columns size={14} /> Copy
            </button>
            <Show when={props.editable}>
              <button class="secondary" onClick={() => setEditing((value) => !value)}>
        ...
        <div class={`json-content ${editing() ? 'editing' : ''}`}>
          {editing() ? (
            <textarea
              value={text()}
              onInput={(event) => {
        ...
        <footer>
          <span>{editing() ? 'Changes are validated before saving' : 'Formatted JSON preview'}</span>
          <div>
