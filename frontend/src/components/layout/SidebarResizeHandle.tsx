...
import type useSidebarPreferences from '../../lib/preferences/useSidebarPreferences'
...
export function useSidebarWidth(
  ...
  min: () => number,
  preferences: ReturnType<typeof useSidebarPreferences>,
) {
  return {
...
export default function SidebarResizeHandle(props: { label: string; sizing: SidebarSizing }) {
  ...
  onCleanup(() => {
    if (drag) document.documentElement.classList.remove('resizing-sidebar')
  })

  return (
    <div
      ref={handle}
      class={`sidebar-resize-handle ${dragging() ? 'dragging' : ''}`}
      role="separator"
      ...
      aria-valuetext={`${sizing.width()} pixels`}
      aria-disabled={sizing.disabled()}
      tabIndex={sizing.disabled() ? -1 : 0}
