...
- An open script is a tab beside the table tabs, and each one owns its text, its last saved copy and its last results. Switching tabs therefore loses nothing and needs no guard; closing a tab with unsaved changes still asks. The SQL button opens a scratch buffer, which has no file behind it and so is never dirty and offers no save.
- A script tab carries its own undo history, capped by the same `editing.undoHistoryLimit` preference the grid drafts use. Writing `value` on a textarea clears its native undo stack, so once the editor sets text programmatically the history has to be ours end to end, including restoring the caret. A run of typing collapses into one step; a pause, a newline, or an edit that is not a single character ends the group.
- The script surface stacks header, editor and results. Every level of that column sets `min-height: 0`, which is what leaves the grid's own scroller with a bounded height; without it the results could not scroll.
